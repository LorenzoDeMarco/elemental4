...
func _on_request_completed(result, response_code, headers, body, handler: HTTPRequest, callbackfn: FuncRef):
...
func request(callback: FuncRef, method: int, url: String, headers: PoolStringArray = PoolStringArray(), body: String = "", token: bool = true) -> int:
	...
	handler.connect("request_completed", self, "_on_request_completed", [handler, callback], CONNECT_DEFERRED | CONNECT_ONESHOT)
